...
//守护进程用路径
func GetGlobalExcPath(defaultPath string) string {
	staticDir := defaultPath
	if len(os.Args) > 1 {
		staticDir = GetExcPath() + "/"
...
func PrintLnLog(a ...interface{}) {
	...
	fmt.Println("["+DateNowFormatStr()+"]", a)
}
...
//UrlValues转Map
func UrlValues2Map(urlData url.Values) map[string]interface{} {
...
func Map2UrlValues(m map[string]interface{}) url.Values {
	...
	for k, v := range m {
		param.Add(k, Interface2string(v))
		//fmt.Println(k,v)
...
func Map2Xml(mReq map[string]interface{}) (xml string) {
	...
	sb.WriteString("<xml>")
	for k, v := range mReq {
		sb.WriteString("<" + k + ">" + Interface2string(v) + "</" + k + ">")
	}
	sb.WriteString("</xml>")
	return sb.String()
}
...
// XML2MAP xml转map
func XML2Map(b []byte) (m map[string]interface{}) {
...
func Map2Struct(mp map[string]interface{}, structs interface{}) {
	err := mapstructure.Decode(mp, &structs)
	if err != nil {
		fmt.Println(err)
	}
}
...
//Map转string
func Map2QueryString(mp map[string]interface{}) string {
	q := ""
	for k, v := range mp {
		if q != "" {
			q += "&"
		}
		q += k + "=" + Interface2string(v)
	}
	return q
}
