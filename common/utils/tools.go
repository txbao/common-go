...
// * 加密 :R([code=>xxx,id=>123]);
// * 返回加密串 :R([code=>xxx,id=>123],true);
// * 解密 :R(string,false);
// params[0] encrypt/decrypt bool true：encrypt false：decrypt, default: false
// params[1] key
func R(text string, params ...interface{}) (str string, err error) {
	l := len(params)
	isEncode := DECODE
	key := ""
	expiry := 0
	...
	if isEncode == ENCODE {
		...
		text = fmt.Sprintf("%s%s", Md5Sum(text + mKey)[0:8], text)
	} else {
		// get strings except dynamic key
		b, e := Base64Decode(text)
		if e != nil {
			return "", e
		}
		text = string(b)
	...
	for i := 0; i < 256; i++ {
		pos := i % keyLen
		rndKey = append(rndKey, int(mKeyB[pos]))
	}
	...
	if isEncode == ENCODE {
		// trim equal
		return strings.Replace(Base64Encode(result), "=", "", -1), nil
	}

	// check expire time
	d := string(result)

	if d[0:8] == Md5Sum(d[8:] + mKey)[0:8] {
		return d[8:], nil
	}
	err = fmt.Errorf("code text [%s] error", text)
	return
}
...
func TimeFormat(t time.Time, f int) (timeStr string) {
	...
	case 5:
		timeStr = t.Format("0601")
	case 6:
		timeStr = t.Format("060102")
	}
...
func IsPrivacyMobile(mobile string) bool {
	reg := `^1([3456789])\d{1}\*{4}\d{4}$`
	rgx := regexp.MustCompile(reg)
