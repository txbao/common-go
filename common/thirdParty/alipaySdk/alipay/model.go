...
//发红包
type AlipayUserDtbankcustChannelvoucherSendResponse struct {
	AlipayCertSn                                   string `json:"alipay_cert_sn"`
	AlipayUserDtbankcustChannelvoucherSendResponse struct {
		ActivityID           string `json:"activity_id"`
		ActivityOrderID      string `json:"activity_order_id"`
		Code                 string `json:"code"`
		DiscountThresholdAmt int64  `json:"discount_threshold_amt"`
		DiscountType         string `json:"discount_type"`
		...
		SubMsg               string `json:"sub_msg"`
		VoucherID            string `json:"voucher_id"`
	} `json:"alipay_user_dtbankcust_channelvoucher_send_response"`
	Sign string `json:"sign"`
}

// 优惠立减信息配置
type DiscountInfoConfig struct {
	DiscountType         string `json:"discount_type"`          // Y 立减类型 reduce:满立减(满5元减3元) random:随机立减(满10元随机立减1元到3元
	DiscountThresholdAmt int64  `json:"discount_threshold_amt"` // Y 优惠立减门槛金额 以分为单位 比如：99代表0.99元
	DiscountValue        string `json:"discount_value"`         // Y 优惠立减值, 分为单位 如果discount_type=reduce，那么就是具体的优惠立减额，比如：999代表满足一定金额减9.99元； 如果discount_type=random，那么就是”最小额-最大额“，比如:99-990，代表立减范围从0.99元到9.99元随机决策
}

// 红包发放时间配置模型
type BenefitSendTime struct {
	SendBeginTime       string   `json:"send_begin_time"`        // Y 红包发放开始时间 示例：2010-11-11 11:11:11
	SendEndTime         string   `json:"send_end_time"`          // Y 红包发放结束时间
	CyclePeriodType     string   `json:"cycle_period_type"`      // N 代表限定周期时间类型 DAY_OF_WEEK:星期维度
	CyclePeriodValue    string   `json:"cycle_period_value"`     // N 代表发放周期类型(cycle_period_type)下具体值。 比如： cycle_period_type=DAY_OF_WEEK，cycle_period_value=["MON","TUE","WEB"], 代表：周一、周二、周三
	SubCyclePeriodType  string   `json:"sub_cycle_period_type"`  // N 代表在周期时间范围下，子周期限定时间类型 SECOND_OF_DAY:一天内具体时间点(秒为单位)
	SubCyclePeriodValue []string `json:"sub_cycle_period_value"` // N 代表发放周期类型(cycle_period_type)下具体值。 比如： cycle_period_type=SECOND_OF_DAY，cycle_period_value=["01:53:28-02:53:28","05:10:10-08:30:30"], 代表： 子时间段为每天的01点53分28秒到02点53分28秒，05点10分10秒到08点30分30秒
}

// 红包发放之后的生效时间配置
type CouponEffectTime struct {
	CouponAvailablePeriodType       string `json:"coupon_available_period_type"`        // Y 时间周期类型 ABSOLUTELY:绝对时间 RELATIVE:相对时间
	CouponAbsolutelyBeginTime       string `json:"coupon_absolutely_begin_time"`        // N 说明:当coupon_available_period_type不为空的时候且为ABSOLUTELY时，此处会有值 绝对时间情况下，红包发放开始时间
	CouponAbsolutelyEndTime         string `json:"coupon_absolutely_end_time"`          // N 说明:当coupon_available_period_type不为空的时候且为ABSOLUTELY时，此处会有值 绝对时间下红包发放结束时间
	CouponRelativeBeginRoundingType string `json:"coupon_relative_begin_rounding_type"` // N 说明:当coupon_available_period_type不为空的时候且为RELATIVE时，此处会有值 相对时间情况下，红包发放开始时间周期类型 DAYS:天级 MINUTES:分钟级
	CouponRelativeBeginValue        int64  `json:"coupon_relative_begin_value"`         // N 说明:当coupon_available_period_type不为空的时候且为RELATIVE时，此处会有值 相对时间情况下，红包开始时间相对值， 比如: coupon_relative_begin_rounding_type=DAYS coupon_relative_begin_value=1 代表次日凌晨开始生效 比如: coupon_relative_begin_rounding_type=MINUTES coupon_relative_begin_value=100 代表当前时间100分钟后开始生效
	CouponRelativeEndRoundingType   string `json:"coupon_relative_end_rounding_type"`   // N 说明:当coupon_available_period_type不为空的时候且为RELATIVE时，此处会有值 相对时间情况下，红包发放结束时间周期类型 DAYS:天级 MINUTES:分钟级
	CouponRelativeEndValue          int64  `json:"coupon_relative_end_value"`           // N 说明:当coupon_available_period_type不为空的时候且为RELATIVE时，此处会有值 相对时间情况下，红包开始时间相对值， 比如: coupon_relative_end_rounding_type=MINUTES coupon_relative_end_value=100 代表从生效时间开始，有效期持续为100分钟
}

// 活动发放和消耗信息
type CouponTemplateConsumeInfo struct {
	ReceiveCount  int64 `json:"receive_count"`  // Y 该活动上已领取红包数量 比如:999代表该活上已被领取999个红包
	ReceiveAmount int64 `json:"receive_amount"` // N 该活动上已领取红包金额，分为单位 999代表已领取9.99元
	UsedCount     int64 `json:"used_count"`     // N 该活动上已核销红包数量 比如:888代表该活上所有领取的红包中已核销了888个红包
	UsedAmount    int64 `json:"used_amount"`    // Y 该活动上已核销红包金额，分为单位 888代表已领取8.88元
	RefundAmount  int64 `json:"refund_amount"`  // Y 该活动上已退款的红包总金额，分为单位 999代表总共退款9.99元
}

// 数字分行红包活动配置查询接口
type AlipayUserDtbankcustChannelvoucherconfigQueryRes struct {
	ErrorResponse
	ActivityID                string                    `json:"activity_id"`                  // Y 数字分行活动唯一id
	ActivityName              string                    `json:"activity_name"`                // Y 渠道红包活动名称描述
	ActivityStatus            string                    `json:"activity_status"`              // Y 活动状态 CREATED="已创建", PUBLISHING="发布中", PUBLISHED_FAILED="发布失败", GREY="已发布(白名单验证中)", GOING="已发布", PAUSED="已暂停", ENDED="已结束"
	BankCardType              string                    `json:"bank_card_type"`               // Y CREDIT:信用卡 DEBIT:借记卡 DEBIT_CREDIT:借记卡或信用卡
	CardBinList               []string                  `json:"card_bin_list"`                // N 银行卡卡bin配置，列表类型,以逗号分隔的字符串数组
	TotalBudget               int64                     `json:"total_budget"`                 // Y 活动总预算值，以分为单位。如果值为999，则说明是9.99元预算
	MinSendCount              int64                     `json:"min_send_count"`               // Y 最少发放红包份数，比如：88代表此活动最少能发88次红包。
	UserInstructionList       []string                  `json:"user_instruction_list"`        // Y 红包使用说明文案
	DiscountInfoConfig        DiscountInfoConfig        `json:"discount_info_config"`         // Y 优惠立减信息配置
	BenefitSendTime           BenefitSendTime           `json:"benefit_send_time"`            // Y 红包发放时间配置模型
	CouponEffectTime          CouponEffectTime          `json:"coupon_effect_time"`           // Y 红包发放之后的生效时间配置
	CouponTemplateConsumeInfo CouponTemplateConsumeInfo `json:"coupon_template_consume_info"` // Y 活动发放和消耗信息 请注意：该结果可能存在2个小时之内的时间延迟，特殊情况下可能延迟会更久
}
type AlipayUserDtbankcustChannelvoucherconfigQueryResponse struct {
	Response     *AlipayUserDtbankcustChannelvoucherconfigQueryRes `json:"alipay_user_dtbankcust_channelvoucherconfig_query_response"`
	AlipayCertSn string                                            `json:"alipay_cert_sn,omitempty"`
	SignData     string                                            `json:"-"`
	Sign         string                                            `json:"sign"`
}
