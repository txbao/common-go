...
// alipay.user.dtbankcust.channelvoucher.send(数字分行红包发放接口)
//	文档地址：https://opendocs.alipay.com/pre-apis/01pnke
func (a *Client) AlipayUserDtbankcustChannelvoucherSend(ctx context.Context, bm gopay.BodyMap) (aliRsp *AlipayUserDtbankcustChannelvoucherSendResponse, err error) {
	err = bm.CheckEmptyError("activity_id", "out_biz_no")
	if err != nil {
		return nil, err
	}
	var bs []byte
	if bs, err = a.doAliPay(ctx, bm, "alipay.user.dtbankcust.channelvoucher.send"); err != nil {
		return nil, err
	}
	fmt.Println("bs", string(bs))
	aliRsp = new(AlipayUserDtbankcustChannelvoucherSendResponse)
	if err = json.Unmarshal(bs, aliRsp); err != nil {
		return nil, err
	}
	if aliRsp.AlipayUserDtbankcustChannelvoucherSendResponse.Code != "10000" {
		//info := aliRsp.AlipayUserDtbankcustChannelvoucherSendResponse
		//return aliRsp, fmt.Errorf(`{"code":"%s","msg":"%s","sub_code":"%s","sub_msg":"%s"}`, info.Code, info.Msg, info.SubCode, info.SubMsg)
		return aliRsp, nil
	}
	signData, signDataErr := a.getSignData(bs, aliRsp.AlipayCertSn)
	aliRsp.Sign = signData
...
// alipay.user.dtbankcust.channelvoucherconfig.query(数字分行红包活动配置查询接口)
// 文档地址: https://opendocs.alipay.com/pre-apis/01trl0
func (a *Client) AlipayUserDtbankcustChannelvoucherconfigQuery(ctx context.Context, bm gopay.BodyMap) (aliRsp *AlipayUserDtbankcustChannelvoucherconfigQueryResponse, err error) {
	err = bm.CheckEmptyError("activity_id")
	if err != nil {
		return nil, err
	}
	var bs []byte
	bs, err = a.doAliPay(ctx, bm, "alipay.user.dtbankcust.channelvoucherconfig.query")
	log.Println("AliConfigQuery", bm, string(bs), err)
	if err != nil {
		return nil, err
	}
	aliRsp = new(AlipayUserDtbankcustChannelvoucherconfigQueryResponse)
	if err = json.Unmarshal(bs, aliRsp); err != nil {
		return nil, err
	}
	if aliRsp.Response != nil && aliRsp.Response.Code != "10000" {
