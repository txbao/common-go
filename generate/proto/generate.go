...
func GenerateSchema(db *sql.DB, table string, ignoreTables []string, serviceName, goPkg, pkg string, isToCamelWithStartLower bool) (*Schema, error) {
	s := &Schema{}
	...
	s.IsToCamelWithStartLower = isToCamelWithStartLower
...
func (s *Schema) String() string {
	...
	for _, m := range s.Messages {
		...
		buf.WriteString("\n")
		m.GenDefaultMessage(buf,s.IsToCamelWithStartLower)
		m.GenRpcAddReqRespMessage(buf,s.IsToCamelWithStartLower)
		m.GenRpcUpdateReqMessage(buf,s.IsToCamelWithStartLower)
		m.GenRpcDelReqMessage(buf)
		m.GenRpcGetByIdReqMessage(buf)
		m.GenRpcSearchReqMessage(buf,s.IsToCamelWithStartLower)
	}
...
//gen default message
func (m Message) GenDefaultMessage(buf *bytes.Buffer,isToCamelWithStartLower bool) {
	mOrginName := m.Name
...
//gen add req message
func (m Message) GenRpcAddReqRespMessage(buf *bytes.Buffer,isToCamelWithStartLower bool) {
	mOrginName := m.Name
	mOrginFields := m.Fields
	...
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"id", "create_time", "update_time", "version", "del_state", "delete_time","created_at","updated_at"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
func (m Message) GenRpcUpdateReqMessage(buf *bytes.Buffer,isToCamelWithStartLower bool) {
	mOrginName := m.Name
	mOrginFields := m.Fields
...
//gen add resp message
func (m Message) GenRpcSearchReqMessage(buf *bytes.Buffer,isToCamelWithStartLower bool) {
	mOrginName := m.Name
	mOrginFields := m.Fields
